...
if (isset($_POST['update_building'])) {
  ...
  $update = mysqli_query($conn, "UPDATE `building` SET `name`='$name',`address`='$address',`built_year`='$built_year',`eer`='$eer',`potential`='$potential',`grade`='$grade' WHERE `id`=$r_id");
        ...
        $grade = mysqli_real_escape_string($conn, $_POST['grade']);
        $r_id = $_POST['id'];
        $update = mysqli_query($conn, "UPDATE `building` SET `name`='$name',`BuildingAddress`='$address', `Building_Type` = '$Building_Type', `built_year`='$built_year',`eer`='$eer',`potential`='$potential',`grade`='$grade' WHERE `id`=$r_id");
...
=======
  <div class="mb-3">
    <label for="address" class="form-label">Address</label>
    <input type="text" name="address" class="form-control" id="address" value="<?=$data['BuildingAddress']?>" required>
  </div>
  <input type="hidden" name="id" value="<?=$data['id']?>">
  <div class="mb-3">
    <label for="built_year" class="form-label">Built Year</label>
