...
if (isset($_GET['id'])) {
    ...
    if (isset($_SESSION['auth'])) {
        if (intval($_SESSION['role']) === 1) { // If user is admin
            $find_calcualtion = mysqli_query($conn, "SELECT * FROM `calculations` WHERE id=$id"); // Check if calculation exists
            if (mysqli_num_rows($find_calcualtion) > 0) { 
                mysqli_query($conn, "DELETE FROM `calculations` WHERE id=$id"); // If exists than delete
